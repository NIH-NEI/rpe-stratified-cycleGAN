from keras.layers import Layer, Input, Conv2D, Activation, add, BatchNormalization
...
class CycleGAN():
    ...
    def __init__(self, image_shape=(256*1, 256*1, 1), epochs = 200, saving_interval = 20,
                 batch_size = 4, cycle_loss_type = 0, lr_D=2e-4, lr_G=2e-4, date_time_string_addition='_cyclegan',
        ...
        self.use_supervised_learning = supervised_learning
    ...
    # def load_input_data(self, input_dir):
    #     # ======= Data ==========
    ...
    #
    #     if self.use_data_generator:
    #         self.data_generator = load_data(data_dir=input_dir, trained_image_size=self.img_shape,
    ...
    #         nr_B_train_imgs = 0
    #
    #     data = load_data(data_dir=input_dir, trained_image_size=self.img_shape, nr_of_channels=self.channels,
    ...
    def load_training_data1(self, spectralis_images, ao_images):
        print('--- Caching data ---')
        sys.stdout.flush()
        ...
        # ======= Create designated run folder and store meta data ==========
        directory = os.path.join('CycleGAN-Images', self.date_time)
        if not os.path.exists(directory):
            os.makedirs(directory)
        self.writeMetaDataToJSON()

    def load_training_data(self, input_dir1, input_dir2):
        # ======= Data ==========
        # Use 'None' to fetch all available images
        nr_A_train_imgs = None
        nr_B_train_imgs = None
        ...
        if self.use_data_generator:
            self.data_generator = load_training_data(trainA_dir=input_dir1, trainB_dir=input_dir2,
                                                     trained_image_size=self.img_shape,
                                                     nr_of_channels=self.channels,
                                                     batch_size=self.batch_size, generator=True)

            # Only store test images
            nr_A_train_imgs = 0
            nr_B_train_imgs = 0

        data = load_training_data(trainA_dir=input_dir1, trainB_dir=input_dir2,
                                  trained_image_size=self.img_shape,
                                  nr_of_channels=self.channels,
                                  batch_size=self.batch_size)
...
# Training
    def train(self, main_frame, epochs,  batch_size=1, save_interval=1):
        ...
        for epoch in range(1, epochs + 1):
            ...
            # main_frame.write_text('DA_losses: {}\n'
            #                       'DB_losses: {}\n'
            #                       'gA_d_losses_synthetic: {}\n'
            #                       'gB_d_losses_synthetic: {}\n'
            #                       'gA_losses_reconstructed: {}\n'
            ...
            #                       'D_losses: {}\n'
            #                       'G_losses: {}\n'
            #                       'reconstruction_losses: {}\n'.format(DA_losses, DB_losses, gA_d_losses_synthetic,
            #                                                            gB_d_losses_synthetic, gA_losses_reconstructed,
            #                                                            gB_losses_reconstructed, D_losses,
            #                                                            G_losses, reconstruction_losses))
    ...
    def saveImages(self, epoch, real_image_A, real_image_B, num_saved_images=1):
        ...
        for i in range(num_saved_images):
            ...
            self.truncateAndSave(real_image_Ab, real_image_A, synthetic_image_B, reconstructed_image_A,
                                 'CycleGAN-Images/{}/{}/epoch{}_sample{}.png'.format(
...
# Save and load
    ...
    def writeLossDataToFile(self, history):
        ...
        with open('CycleGAN-Images/{}/loss_output.csv'.format(self.date_time), 'w') as csv_file:
    ...
    def writeMetaDataToJSON(self):
        ...
        with open('CycleGAN-Images/{}/meta_data.json'.format(self.date_time), 'w') as outfile:
    ...
    def load_model_and_generate_synthetic_images(self, weight_path, weight_path1, dirA_name, dirB_name, main_frame):
        ...
        for i in range(len(synthetic_images_A)):
            ...
            name = self.testB_image_names[i].strip('.tif') + 'training' + str(samples) + '_iterations' + str(epoch) \
                   + '_synthetic.png'
            synt_A = synthetic_images_A[i]
            save_image(synt_A, name, dirA_name)
            main_frame.show_progress(float(i+1) / float(len(synthetic_images_A)+len(synthetic_images_B)) * 100)

        # Test B images
        directory = os.path.join('generate_images', self.date_time, dirB_name)
        if not os.path.exists(directory):
            os.makedirs(directory)
        for i in range(len(synthetic_images_B)):
            # Get the name from the image it was conditioned on
            name = self.testA_image_names[i].strip('.tif') + 'training' + str(samples) + '_iterations' + str(epoch) \
                   + '_synthetic.png'
            synt_B = synthetic_images_B[i]
            save_image(synt_B, name, dirB_name)
            main_frame.show_progress(float(i+len(synthetic_images_A)+1) /
                                     float(len(synthetic_images_A) + len(synthetic_images_B)) * 100)

        main_frame.write_text('{} synthetic images have been generated and placed in ./generate_images/synthetic_images'
